...
n_UG <-
  TD_UG %>% group_by(ind_name, month) %>% dplyr::summarise(Freq = n())
...
n_temporalthinning <- TD_track_tempFilter %>%
  group_by(ind_id, month) %>% dplyr::summarise(Freq = n())

# Spatially filter the location data to 1 km ------------------------------

names(TD_track_tempFilter) <- c("lng", "lat", "time", "ind_id", "month")
...
TD_hq_notused <-
  TD_filtered_df[!(TD_filtered_df$lng %in% TD_samples$lng & TD_filtered_df$lat %in% TD_samples$lat),]
...
n_ind <- rbind(n_UG, n_dn, n_temporalthinning, n_spatiotemporalthinning)
...
n_ind_extrema <- n_ind_extrema %>%arrange(Freq) %>%  mutate(month = factor(month, levels=c("9", "10", "11", "12", "1", "2", "3")))
  

pd <- position_dodge(0.6)

ggplot(data = n_ind_extrema, aes(x = Freq, y = as.factor(step), colour = as.factor(month))) +
  theme_set(new = theme_bw()) +
  geom_errorbar(aes(xmin = n_min, xmax = n_max), position = pd, width = 0.5) +
