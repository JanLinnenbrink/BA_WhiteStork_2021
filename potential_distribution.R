...
mpiab_argos$speed <- unlist(lapply(speed(mpiab_argos),c, NA ))
...
b <- elev %>% mask(known_distr) #elev as background raster
td_sf <- st_as_sf(TD_wo_outl, coords = c("location.long", "location.lat"),
                  crs = crs)

td_ug <- raster::extract(x=b, y=td_sf)
td_sf$vals <- td_ug
td_sf <- td_sf[!is.na(td_sf$vals),]
TD <- st_drop_geometry(td_sf)
TD$x <- st_coordinates(td_sf)[,"X"]
TD$y <- st_coordinates(td_sf)[,"Y"]
...
TD_amt_spl <- split(TD_amt, TD_amt$ind.id, drop = TRUE)
TD_amt_l <- TD_amt_spl[lapply(TD_amt_spl, nrow)>40]

# filter flyway

TD_amt_l <- lapply(TD_amt_l, function(x) {x$speed <- amt::speed(x); return(x)})

TD_amt_l1 <- TD_amt_spl[lapply(TD_amt_spl, nrow)>40]

# calculate distance between the data points
TD_l <- lapply(TD_amt_l1, function(x){ x[order(x$t_),]})

TD_l <- lapply(TD_l, function(x) { mutate(x, 
                                          distance=distHaversine(cbind(x_, y_),
                                                                 cbind(lag(x_), lag(y_))))})

# calculate time difference between observations
TD_l <- lapply(TD_l, function(x){ mutate(x, diff = t_ - lag(t_),
                                         diff_sec = as.numeric(diff, units = "secs"))})

# calculate speed
TD_l <- lapply(TD_l, function(x) { x$speed <- x$distance / x$diff_sec; return(x)})

# filter ground records (speed < 2 m/s)
TD_l <- lapply(TD_l, filter, speed < 2)

# resting points as amt
TD_rest <- ungroup(do.call(rbind.data.frame, TD_l))

# Remove all locations from night except one
TD_track <- time_of_day(TD_rest)
...
TD_track <- select(TD_track, -c(7:11))
...
saveRDS(TD_samples, paste0(path_out_pop, "occs_pot_ug_new"))
TD_samples <- readRDS(paste0(path_out_pop, "occs_pot_ug_new"))
...
occ_sp <- readRDS(paste0(path_out_pop, "occs_pot_ug_new"))
...
par(mfrow=c(2,2))
plot(b)
points(filter(TD_samples, TD_samples$month==10))
plot(b)
points(filter(TD_samples, TD_samples$month==12))
plot(b)
points(filter(TD_samples, TD_samples$month==1))
plot(b)
points(filter(TD_samples, TD_samples$month==2))
...
saveRDS(models_ndvi, paste0(path_out_pop, "models_ndvi"))
...
for(i in seq_along(models_ndvi)) {
  preds_ndvi[[i]] <- dismo::predict(object=models_ndvi[[i]], x=env_ndvi_list[[i]], type = 'cloglog', 
                        filename = paste0(path_preds, "preds_ndvi", i, ".tiff"), overwrite = TRUE)
}
...
ef_df <- as.data.frame(Bavaria)
n.storks <- length(unique(ef_df$individual.id))
ind_name <- paste0("EAS", 1:n.storks)
ef_df$ind_name <- factor(ef_df$individual.id, labels = ind_name)
ef_df <- ef_df[, (names(ef_df) %in% keep_cols)]

ef <- ef_df %>% subset(location.long>-50 & location.long <50 & location.lat>-40)


# Clip tracking data to study area
path_known_distr <- "C:/1_Bachelorarbeit/BirdLife_Verbreitungskarte/"
known_distr <- readOGR(paste0(path_known_distr, "Ciconia_ciconia.shp"))

ef$inUG <- factor(point.in.polygon(
  ef$location.long,
  ef$location.lat,
  UG$x,
  UG$y
...
test_UG <- filter(ef, ef$inUG == 1)
test_UG$inUG <- NULL

test_UG$year <- year(test_UG$timestamp)
test_UG$month <- month(test_UG$timestamp)
test_10_3 <- test_UG %>% filter(month != 9)
...
# filter flyway
test_amt_spl <- split(test_amt, test_amt$ind.id, drop = TRUE)
test_amt_l <- test_amt_spl[lapply(test_amt_spl, nrow)>40]
test_amt_l <- lapply(test_amt_l, function(x) {x$speed <- amt::speed(x); return(x)})

test_amt_l1 <- test_amt_spl[lapply(test_amt_spl, nrow)>40]

# calculate distance between the data points
test_l <- lapply(test_amt_l1, function(x){ x[order(x$t_),]})

test_l <- lapply(test_l, function(x) { mutate(x, 
                                              distance=distHaversine(cbind(x_, y_),
                                                                     cbind(lag(x_), lag(y_))))})

# calculate time difference between observations
test_l <- lapply(test_l, function(x){ mutate(x, diff = t_ - lag(t_),
                                             diff_sec = as.numeric(diff, units = "secs"))})

# calculate speed
test_l <- lapply(test_l, function(x) { x$speed <- x$distance / x$diff_sec; return(x)})

# filter ground records (speed < 2 m/s)
test_l <- lapply(test_l, filter, speed < 2)

# resting points as amt
test_rest <- do.call(rbind.data.frame, test_l)

# Remove all locations from night except one
test_track <- time_of_day(test_rest)

test_track$day <- substr(x = test_track$t_, start = 0, stop = 10)

test_night <- test_track %>%
  filter(tod_ == "night") %>%
  group_by(ind.id, day, year) %>%
  slice(1)
...
test_track <- full_join(test_day, test_night) %>%
  select(-day)

test_track <- select(test_track, -c(7:11))

# temporal thinning
test_track$groupsTime <- cut(test_track$t_, breaks = "90 mins")

test_track_tempFilter <- test_track %>%
  group_by(groupsTime, ind.id) %>%
  filter(row_number() == 1)

test_track_tempFilter$tod_ <- NULL
test_track_tempFilter$groupsTime <- NULL

n_temporalthinning <- test_track_tempFilter %>%
  group_by(ind.id, month) %>%
  dplyr::summarise(Freq = n())
colnames(n_temporalthinning) <- c("ind_id", "month", "Freq")

# Spatially filter the location data to 1 km
names(test_track_tempFilter) <- c("lng", "lat", "time", "ind_id", "month", "year")

test_g <- test_track_tempFilter %>% split(list(test_track_tempFilter$ind_id,
                                               test_track_tempFilter$month, 
                                               test_track_tempFilter$year),
                                          drop = TRUE)
...
spatialFilter <- function(df, dist) {
  ...
  crs(df) <- crs
  df_tr <- spTransform(df, crs_new)
  test_sT2 <- remove.near(df_tr, dist = dist)
}

test_SpT <- lapply(test_g, spatialFilter, dist = thin_distance)

test_SpT_SPDF <- do.call(rbind.SpatialPointsDataFrame, test_SpT) %>% spTransform(crs) 
...
#remove not existing levels of ind_id
test_threshold$ind_id <- factor(test_threshold$ind_id)
...
test_samples <-
  ...
  as.data.frame() %>%
  select(-year) 
test_samples$ind_id <- factor(test_samples$ind_id)

coordinates(test_samples) <- ~ lng + lat
crs(test_samples) <- crs

saveRDS(test_samples, paste0(path_out_pop, "test_samples"))


### evaluation on new data

test_m <- split(test_samples, test_samples$month)
...
# calculate test metrics with independent test data -------------------------

dir.create(paste0(path_out_pop, "independent_evaluation"))
path_ind_ev <- paste0(path_out_pop, "independent_evaluation/")
...
t_or <- mapply(extract, list(preds_10, preds_11, preds_12, preds_01, preds_02),
               list(test_10, test_11, test_12, test_01, test_02))

t_truep <- lapply(t_or, function(x){x[x == 1]})
t_falsen <- lapply(t_or, function(x){x[x == 0]})

n_truepositive <- lapply(t_truep, length)
n_falsenegative <- lapply(t_falsen, length)

rate_truep <- mapply(function(x, y){x/(x+y)}, n_truepositive, n_falsenegative)

or <- 1 - rate_truep

or_df <- data.frame("month" = factor(c(10,11,12,01,02)), 
                     "true_positives" = unlist(n_truepositive),
                     "false_negatives" = unlist(n_falsenegative),
                     "true_positive_rate" = rate_truep, 
                     "omission_rate" = or)


or_melt <- or_df %>% 
  select(c("month", "true_positives", "false_negatives")) %>% 
  melt(id.vars = "month")

tp <- ggplot(or_melt, mapping = aes(x = month, y = value, fill = variable)) +
  geom_bar(stat = "identity", position = "dodge") +
  xlab("month") + ylab("Vorkommenspunkte") +
  theme_bw()

ggsave(paste0(path_ind_ev, "ind_ev.png"), tp, device = "png")

write.xlsx(or_df, paste0(path_ind_ev, "ind_ev.xlsx"))
