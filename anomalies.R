...
colnames(prec.df) <- c("Niederschlag", "Jahr", "Monat")
...
prec.mean.y$Niederschlagsanomalie <-
  (prec.mean.y$Niederschlag - prec.mean.20) / sd(prec.mean.y$Niederschlag)
...
colnames(temp.df) <- c("Temperatur", "Jahr", "Monat")
...
temp.mean.y$Temperaturanomalie <-
  (temp.mean.y$Temperatur - temp.mean.20) / sd(temp.mean.y$Temperatur)
